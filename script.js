...
const HERO_LINES = [
  {
    prompt: "$ whoami",
    output: "raza_kashif — Test Analyst / QA Automation Engineer / SDET",
  },
  {
    prompt: "$ suite --health BiblioSuite",
    output: "recovered 15–20% → 95–98% pass rate (CI-verified)",
  },
  {
    prompt: "$ releases --owned-solo",
    output: "4 / 6 — regression + system testing, end-to-end",
  },
...
const EXPERIENCE_STEPS = [
  {
    action:
      "Completed structured onboarding (Java, Selenium WebDriver, TestNG, Apache POI, SQL, API testing, DevOps tooling)",
    result: "Ready for OUP BiblioSuite project assignment",
  },
  {
    action:
      "Inherited a legacy automation suite averaging just 15–20% pass rate — brittle locators, flawed logic",
    result:
      "Rebuilt core logic + dynamic, resilient locator strategy from the ground up",
  },
  {
    action:
      "Maintained suite health across releases despite routine UI and application-side changes",
    result:
      "Sustained a 95–98% pass rate, verified through daily CI run reports",
  },
  {
    action:
      "Senior teammate departed mid-project, leaving a release-ownership gap",
    result: "Took over full release ownership independently",
  },
  {
    action:
      "Owned 4 of the team's last 6 releases solo — regression + system testing, incl. overnight pushes",
    result: "Zero missed deadlines, recognized by client and management",
  },
  {
    action:
      "Delivered hands-on onboarding training (JS, Node.js, WebdriverIO, locators, OAF, full BiblioSuite workflow) to new Graduate Engineer Trainees",
    result: "Faster ramp-up for newly onboarded engineers",
  },
  {
    action:
      "API testing (Postman), test-artifact management (Jira/Xray, RTM), CI integration (Jenkins, GitHub Actions)",
    result: "Audit-ready documentation, continuous testing on every push",
  },
  {
    action:
      "Applied prompt engineering + AI coding assistants (Claude, GitHub Copilot) to accelerate test authoring and script scaffolding",
    result: "Faster turnaround, full manual review and ownership retained",
  },
];
...
const SKILL_GROUPS = [
  ...
  {
    label: "Languages",
    items: ["JavaScript (ES6+)", "Java", "SQL", "Python (basics)"],
  },
  {
    label: "CI/CD & DevOps",
    items: ["Jenkins", "GitHub Actions", "Git", "GitHub", "GitLab", "Docker"],
  },
  {
    label: "Test Management",
    items: ["Jira", "Xray", "Apache POI", "RTM", "Defect Lifecycle"],
  },
  {
  ...
  {
    label: "Methodologies",
    items: ["Agile/Scrum", "SDLC", "STLC", "Sprint Planning"],
  },
];
...
const PROJECTS = [
  {
    ...
    points: [
      "End-to-end framework: auth, search, cart, order placement, payment validation",
      "Data-driven layer via Apache POI, multi-scenario Excel datasets — 3x functional coverage",
      "Cross-browser regression across Chrome, Firefox, Edge — zero UI regressions",
    ],
  },
  {
    id: "TC-04.2",
    title: "Insurance Portal Test Automation",
    tag: "Practice Project",
    dev: false,
    stack: "Selenium WebDriver · TestNG · POM · Jenkins CI/CD",
    points: [
      "Modular Selenium + TestNG + POM framework for search, form validation, comparison flows",
      "Jenkins CI/CD integration for scheduled and on-demand execution",
      "Negative and boundary testing surfaced critical input defects before UAT",
    ],
  },
  {
    id: "TC-04.3",
    title: "Bike Rental Management System",
    tag: "Dev Project — Final Year (BCA)",
    dev: true,
    stack: "JavaScript · Node.js · Database",
    points: [
      "Full-stack web application — not a testing project, a self-built product",
      "End-to-end booking, return, and payment modules",
      "Built solo as a final-year academic project",
    ],
  },
];
...
const CERTIFICATIONS = [
  {
    ...
    issuer: "Anthropic · Credly-verified",
    date: "Jul 2026",
  },
  {
    name: "GenAI & AI Agents for QA Automation",
    issuer: "Copilot & Claude Code · Rahul Shetty Academy",
    date: "Jun 2026",
  },
  {
    name: "Gen AI Test Automation",
    issuer: "Selenium, Playwright, Copilot, Claude · Firstlink Consulting",
    date: "Jun 2026",
  },
  {
    name: "Cognizant Analyst Trainee Program",
    issuer: "QA & Automation · Cognizant Technology Solutions",
    date: "2025",
  },
  {
    name: "Namaste JavaScript — Advanced JS (ES6+)",
    issuer: "Akshay Saini",
    date: "2025",
  },
];
...
const RECOGNITION = [
  '"Tester of the Month" ×2',
  '"Rising Star" Award ×2',
  "Highest Quarterly Performance Rating",
  "Promoted: Analyst Trainee → Analyst (Apr 2026)",
];
...
function setPipelineState(activeId) {
  ...
  STAGES.forEach((s, i) => {
    ...
    futureIcon.style.display = "none";
    if (i < idx) {
      checkIcon.style.display = "block";
      label.className = "stage-label past";
    } else if (i === idx) {
      activeIcon.style.display = "block";
      label.className = "stage-label active";
    } else {
      futureIcon.style.display = "block";
      label.className = "stage-label future";
    }
    if (i < STAGES.length - 1) {
...
termBody.innerHTML = HERO_LINES.map(
  (l, i) => `
    <div class="term-line">
...
try {
  ...
  /* purely decorative — safe to skip */
}
...
try {
  ...
  /* if this fails, .reveal elements simply stay visible */
}
...
try {
  ...
  /* if this fails, the static final numbers already show */
}
